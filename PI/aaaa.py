...
df = pd.read_csv(r'C:\Users\Aluno\Desktop\PI\experimento.csv')


df['TEMPERATURA'] = df['TEMPERATURA'].str.replace('°C', '').astype(float)
df['CONDUTIVIDADE'] = df['CONDUTIVIDADE'].str.replace(',', '.').astype(float)
df['ALCALINIDADE TOTAL'] = df['ALCALINIDADE TOTAL'].str.replace(',', '.').astype(float)


if 'ACIDEZ' in df.columns:
    df['ACIDEZ'] = df['ACIDEZ'].str.replace(',', '.').astype(float)
else:
    st.warning("A coluna de acidez não foi encontrada no DataFrame.")
    df['ACIDEZ'] = None  
...
fig_pizza = px.pie(temperature_counts, names='TEMPERATURA', values='CONTAGEM',
                   title='Distribuição das Temperaturas das Amostras')


fig_condutividade = px.bar(df, x='NOME', y='CONDUTIVIDADE',
...
fig_dual_axis.add_trace(go.Scatter(x=df['NOME'], y=df['ALCALINIDADE TOTAL'], mode='lines', name='Alcalinidade Total',
                                  line=dict(color='blue'), yaxis='y1'))


if df['ACIDEZ'].isnull().sum() < len(df):  
    fig_dual_axis.add_trace(go.Scatter(x=df['NOME'], y=df['ACIDEZ'], mode='lines', name='Acidez',
...
temp_filter = st.slider('Selecione a faixa de Temperaturas', min_temp, max_temp, (min_temp, max_temp))


df_filtered = df[(df['TEMPERATURA'] >= temp_filter[0]) & (df['TEMPERATURA'] <= temp_filter[1])]


fig_map_filtered = px.scatter_geo(df_filtered, lat='LATITUDE', lon='LONGITUDE',
                                  hover_name='NOME', hover_data=['LOCAL', 'TEMPERATURA'],
...
fig_pizza_filtered = px.pie(temperature_counts_filtered, names='TEMPERATURA', values='CONTAGEM',
